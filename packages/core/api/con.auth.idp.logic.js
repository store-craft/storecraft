...
import { isAdminEmail, sanitize_auth_user, upsert_auth_user } from './con.auth.logic.js'
import { OAuth } from '../crypto/oauth-1.0a.js'
import pkceChallenge from '../crypto/pkce-challenge.js'
import { uuid } from '../crypto/short-uuid.js'
 ...
 * @param {App} app 
 */  
export const create_idp_auth_uri_for_webapps = (app) => 
  /**
   * 
  ...
  async (params) => {
    ...
    /** @type {ReturnType<providers>[keyof ReturnType<providers>]} */
    const provider = providers(app)?.[params?.provider];
...
export const sign_with_identity_provider = (app) => 
  ...
  async (params) => {
    ...
    const response = await provider.sign_with_authorization_response(
      {
        provider: params.provider,
        redirect_uri: params.redirect_uri,
 ...
 * 
 * @param {App} app 
 * @returns {import('./con.auth.idp.types.js').OAuthProvider[]}
 */  
export const identity_providers = (app) => {
  return Object.entries(providers(app)).map(
    ([handle, value]) => (
      {
        provider: handle,
        name: value.name,
