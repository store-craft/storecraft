...
async (body) => {
  ...
  const confirm_email_token = await jwt.create(
    ...
    { sub: id, id, aud: CONFIRM_EMAIL_TOKEN },
    jwt.JWT_TIMES.WEEK
  );
...
export const forgot_password_request = (app) => 
  ...
  async (email) => {
    ...
    const token = await jwt.create(
      ...
      { sub: email, email, aud: FORGOT_PASSWORD_IDENTITY_TOKEN },
      jwt.JWT_TIMES.HOUR
    );
...
export const forgot_password_request_confirm = (app) => 
  /**
   * @description User has supplied the `token` generated by the 
   * {@link forgot_password_request},
