/**
 * @import {
 *  ApiQuery, Cursor, SortCursor, 
 *  ExpandQuery, SortOrder, Tuple
...
export const parse_query = (s) => {
  ...
  { // LEGACY query cursor, will be deprecated
    ...
    if(vql_string) {
      const older = parenthesise_vql_string(s.get(VQL) ?? '');
...
export const api_query_to_searchparams = (q) => {
  ...
  // console.log(q);
  { // LEGACY query cursor, will be deprecated
    const legacy_to_vql = legacy_query_to_vql_string(q);
    const compiled_current_vql = q.vql && compile(q.vql);
    q.vql = parenthesise_vql_string(
      legacy_to_vql + ' & ' + compiled_current_vql
    )
  }
  // [
