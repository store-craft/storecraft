import { ProductData, ShippingData, 
  ...
  LineItem, FilterMeta, PricingData, BuyXGetYDiscountExtra} from '../../js-docs-types.js'
...
export const calculate_line_items_discount_with_bulk_discount = 
  ...
  if(qty <= 0)
    throw new Error('bulk discount qty <= 0')

  // compute all the total quantity that is legable for the
  // discount
  const total_legal_qty = line_items.reduce(
    (p, c, ix) => p + (pass_mask[ix] ? c.qty : 0) , 0
  )
  ...
  const report = line_items_next.reduce(
    (p, c, ix, array) => {
      if(!pass_mask[ix])
        return p
  ...
  const total_discount = apply_discount(
    1, report.total, $percent, $fixed * how_many_bulks_to_fit
    )
...
export const calculate_line_items_discount_with_buy_x_get_y_discount = 
  ...
  do {
    ...
    const { line_items_next: line_items_x_next } = reduce_from_line_items(
      result.line_items, qty_x, pass_mask_x
    )
