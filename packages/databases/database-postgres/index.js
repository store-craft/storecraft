...
export class Postgres extends SQL {
  ...
  init = (app) => {
    this.pg_config.pool_config.host ??= 
      app.platform.env[EnvConfig.pool_config.host];

    this.pg_config.pool_config.port ??= 
      parseFloat(app.platform.env[EnvConfig.pool_config.port]);

    this.pg_config.pool_config.user ??= 
      app.platform.env[EnvConfig.pool_config.user];

    this.pg_config.pool_config.password ??= 
      app.platform.env[EnvConfig.pool_config.password];
