...
```js
const product_by_id = await app.api.collections.get('col_65e5ca42c43e2c41ae5216a9');
const product_by_handle = await app.api.collections.get('nintendo-games');
```
...
```js
const products = await app.api.collections.list(
  {
    startAfter: [
