...
and have tags `tag:genre_action` OR `tag:genre_adventure`
- In the future cursor based `startAfter` / `endAt` will be deprecated in favor of `vql` based queries
such as `created_at>=2023-01-01T00:00:00Z & active=true`
...
const products = await app.api.products.list(
  {
    ...
    vql: '-col:collection-a & (tag:genre_action | tag:genre_adventure)',
    limit: 5
  }
);
