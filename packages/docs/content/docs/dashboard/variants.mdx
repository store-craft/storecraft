...
Product variants is a staple feature every merchant should know about and use.
But, what are they exactly ?

So, let's suppose you are selling a branded shirt 👕 and let's suppose that the shirt
...
`shelf` supports product variants.
With `shelf`, we have a main product and each variant is a child product with it's own handle
